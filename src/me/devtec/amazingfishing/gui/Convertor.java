...
public class Convertor {
	...
	public static void open(Player p) {
		boolean withCloseState = Loader.config.getString("Options.Convertor.BackButton-Action").equalsIgnoreCase("CLOSE");
		...
		}, Create.make("convertor.close").build(), !withCloseState ? Help::open : null, withCloseState ? withClose : withoutClose);
		a.setItem(49, new ItemGUI(Create.make("convertor.convert").build()) {
			...
			private void convert(HolderGUI arg1) {
				for (int count = 0; count < 4; ++count)
					if (arg1.getItem(count) == null || arg1.getItem(count).getType() == Material.AIR)
						continue;
					else if (API.isAFItem(arg1.getItem(count)) && API.isConvertable(arg1.getItem(count)))
			...
			private int find(HolderGUI arg1, ItemStack stack, int count) {
				if (stack != null)
					if (arg1.getItem(count) == null || arg1.getItem(count).getType() == Material.AIR
							|| arg1.getItem(count).getType() == stack.getType() && arg1.getItem(count).getAmount() != 64 && arg1.getItem(count).getItemMeta().equals(stack.getItemMeta()))
						return count;
				return -1;
			}
		});
