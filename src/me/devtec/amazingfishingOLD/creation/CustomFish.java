...
import me.devtec.amazingfishingOLD.utils.Utils;
...
public class CustomFish implements Fish {
	...
	public ItemStack createItem(double weight, double length) {
		ItemMaker c = Create.find(item, type.name(), type.ordinal());
		String bc = sub(getBiomes().toString()), bbc = sub(getBlockedBiomes().toString()), cf = getDisplayName().replace("%weight%", Loader.ff.format(weight))
		...
		l.replaceAll(a -> PlaceholderAPI.apply(a.replace("%weight%", Loader.ff.format(weight)).replace("%length%", Loader.ff.format(length)).replace("%chance%", Loader.ff.format(getChance()))
				.replace("%name%", cf).replace("%biomes%", bc).replace("%blockedbiomes%", bbc), null));
		c.lore(l);
	...
	public ItemStack createItem(double weight, double length, double money, double points, double exp) {
		...
		String bc = sub(getBiomes().toString()), bbc = sub(getBlockedBiomes().toString()),
				cf = getDisplayName().replace("%weight%", Loader.ff.format(weight)).replace("%length%", Loader.ff.format(length)).replace("%chance%", Loader.ff.format(getChance()))
						.replace("%biomes%", bc).replace("%blockedbiomes%", bbc).replace("%money_boost%", Loader.ff.format(money)).replace("%points_boost%", Loader.ff.format(points))
						.replace("%exp_boost%", Loader.ff.format(exp)).replace("%money_bonus%", Loader.ff.format(money)).replace("%points_bonus%", Loader.ff.format(points))
						.replace("%exp_bonus%", Loader.ff.format(exp)).replace("%name%", getName());
		c.displayName(cf);
		List<String> l = data.getStringList(path + "." + name + ".lore");
		l.replaceAll(a -> PlaceholderAPI.apply(a.replace("%weight%", Loader.ff.format(weight)).replace("%length%", Loader.ff.format(length)).replace("%chance%", Loader.ff.format(getChance()))
				.replace("%name%", cf).replace("%biomes%", bc).replace("%blockedbiomes%", bbc).replace("%money_boost%", Loader.ff.format(money)).replace("%points_boost%", Loader.ff.format(points))
				.replace("%money_bonus%", Loader.ff.format(money)).replace("%points_bonus%", Loader.ff.format(points)).replace("%exp_bonus%", Loader.ff.format(exp))
				.replace("%exp_boost%", Loader.ff.format(exp)), null));
		c.lore(l);
		for (String enchant : getEnchantments())
			if (EnchantmentAPI.byName(enchant) != null)
				c.enchant(EnchantmentAPI.byName(enchant).getEnchantment(), 1);
	...
	public ItemStack createItem(double weight, double length, double money, double points, double exp, Player p, Location hook) {
		ItemMaker c = Create.find(item, type.name(), type.ordinal());
		String bc = sub(getBiomes().toString()), bbc = sub(getBlockedBiomes().toString()),
				cf = s(getDisplayName(), p, hook).replace("%weight%", Loader.ff.format(weight)).replace("%length%", Loader.ff.format(length)).replace("%chance%", Loader.ff.format(getChance()))
						.replace("%biomes%", bc).replace("%blockedbiomes%", bbc).replace("%money_boost%", Loader.ff.format(money)).replace("%points_boost%", Loader.ff.format(points))
						.replace("%exp_boost%", Loader.ff.format(exp)).replace("%money_bonus%", Loader.ff.format(money)).replace("%points_bonus%", Loader.ff.format(points))
						.replace("%exp_bonus%", Loader.ff.format(exp)).replace("%name%", getName());
		c.displayName(cf);
		List<String> l = data.getStringList(path + "." + name + ".lore");
		l.replaceAll(a -> s(a.replace("%weight%", Loader.ff.format(weight)).replace("%length%", Loader.ff.format(length)).replace("%chance%", Loader.ff.format(getChance())).replace("%name%", cf)
				.replace("%biomes%", bc).replace("%money_boost%", Loader.ff.format(money)).replace("%points_boost%", Loader.ff.format(points)).replace("%exp_boost%", Loader.ff.format(exp))
				.replace("%money_bonus%", Loader.ff.format(money)).replace("%points_bonus%", Loader.ff.format(points)).replace("%exp_bonus%", Loader.ff.format(exp)).replace("%blockedbiomes%", bbc), p,
				hook));
		c.lore(l);
		for (String enchant : getEnchantments())
			if (EnchantmentAPI.byName(enchant) != null)
	...
	public ItemStack preview(Player p) {
		ItemMaker c = Create.find(showItem, type.name(), type.ordinal());
		String bc = sub(getBiomes().toString()), bbc = sub(getBlockedBiomes().toString());
